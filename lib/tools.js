module.exports = {
  ...
  replaceEnvValue: function (recipe) {
    let recipeStr = JSON.stringify(recipe);
    const envs = recipeStr.match(/\${.*?\}/g);
    if (envs) {
      envs.forEach((env) => {
        const envValue = process.env[env.replace(/\${|}/g, '')];
        recipeStr = recipeStr.replace(env, envValue);
      });
    }
    return JSON.parse(recipeStr);
  },
};
