...
module.exports = {
  ...
  'buildHostConfig': function (service, recipe, filename) {
    var output = {
    ...
    if (service.volumes_from !== undefined) {
      for (var volume_from of service.volumes_from) {
        ...
        this.buildVolumesHostconfig(filename,svf.volumes, output, vf[1]);
      }
    }

    if (service.volumes !== undefined) {
      this.buildVolumesHostconfig(filename,service.volumes, output);
    }
  ...
  'buildVolumesHostconfig': function (filename, volumes, output, type) {
    ...
    for (var volume of volumes) {
      if (typeof volume === 'string' || volume instanceof String) {
        var sourceTarget = volume.split(':', 2);
        var aux = path.resolve(base, sourceTarget[0]) +":"+ sourceTarget[1];
        if (type == 'ro') {
          aux += ':ro';
        }
        ...
        if (volume.source && volume.target) {
          volume.source =path.resolve(base, volume.source)
