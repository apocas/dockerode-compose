...
var extendsServices = function (service, recipe) {
  ...
  if (service.extends.file === undefined) {
    ...
    return mergeExtendsService(service, service.extends.service, recipe);
  } else {
    // EXTENDS OF ANOTHER RECIPE
    try {
      var extendsRecipe = yaml.load(fs.readFileSync(service.extends.file, 'utf8'));
      return mergeExtendsService(service, service.extends.service, extendsRecipe);
    } catch (e) {
      throw e;
    }
  }
...
var mergeExtendsService = function (service, extendsServiceName, recipe) {
  var restrictions = [
    'links',
    'volumes_from',
    'depends_on',
  ...
  for (var extendsRecipeServiceName of extendsRecipeServiceNames) {
    if (extendsRecipeServiceName == extendsServiceName) {
      ...
      for (var key of oldServiceKeys) {
        // TODO: IMPROVE/IMPLEMENT Merging service definitions
        if (key != 'extends') {
          Object.assign(service[key], oldService[key]); // KINDA MAPPINGS
          // SEQUENCES
          //service[key] = oldService[key]; // SCALARS
        }
      }
      return service;
    }
