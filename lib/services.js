    ...
    if (service.build !== undefined) {
      ...
      if (typeof service.build === 'object') {
        if (service.build.context !== undefined) {
          ...
          if (fs.existsSync(buildContextPath)) {
            ...
            if (service.build.dockerfile === undefined) {
              await buildDockerImage(docker, buildContextPath, obj, null, options);
            } else {
              await buildDockerImage(docker, buildContextPath, obj, service.build.dockerfile, options);
            }
          } else {
        ...
        if (fs.existsSync(dockerFilePath)) {
          await buildDockerImage(docker, dockerFilePath, obj, null, options);
        } else {
...
var buildDockerImage = async function (docker, buildPath, obj, dockerfile, options) {
  options = options || {};
  if (dockerfile !== null) {
    obj['dockerfile'] = path.basename(dockerfile);
    try {
      var streami = await docker.buildImage({
        context: buildPath,
        src: [dockerfile]
      }, obj);
    ...
    try {
      var streami = await docker.buildImage(tarStream, obj);
      if (options.verbose === true) {
        streami.pipe(process.stdout);
      } else {
        streami.pipe(stream.PassThrough());
