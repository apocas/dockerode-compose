  ...
  for (var serviceName of serviceNames) {
    ...
    if (service.extends !== undefined) {
      if (service.extends.service !== undefined) {
        service = extendsServices(service, recipe)
      } else {
    ...
    if (service.build !== undefined) {
      //BUILD AN IMAGE
      var absolutePath = path.dirname(output.file);
        ...
        if (service.build.context !== undefined) {
          ...
          if (fs.existsSync(buildContextPath)) {
            if (service.build.dockerfile !== undefined) {
              buildContextPathWDockerfile = path.resolve(path.join(buildContextPath, service.build.dockerfile));
            }
            console.log(buildContextPath)

            if (buildContextPathWDockerfile == "") {
              await buildDockerImage(docker, buildContextPath, obj, null);
            } else {
              await buildDockerImage(docker, buildContextPathWDockerfile, obj, [service.build.dockerfile]);
            }
          } else {
...
var extendsServices = function (service, recipe) {
  // there will be problems with non absolute paths (service.extends.file)
  // with the build implementation we have the "absolute docker-compose path" (var absolutePath = path.dirname(output.file))

  // TODO: IMPLEMENT THE ABSOLUTE DOCKER-COMPOSE PATH 
  ...
  if (service.extends.file === undefined) {
    ...
    return buildExtendsService(service, service.extends.service, recipe);
  } else {
    // EXTENDS OF ANOTHER RECIPE
    try {
      var extendsRecipe = yaml.load(fs.readFileSync(service.extends.file, 'utf8'));
      return buildExtendsService(service, service.extends.service, extendsRecipe);
    } catch (e) {
      throw e;
    }
  }
...
var buildExtendsService = function (service, extendsServiceName, recipe) {
  var extend = false;
...
var buildDockerImage = async function (docker, buildPath, obj, dockerfiles) {
