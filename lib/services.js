...
var buildHostConfig = function (service, recipe) {
  ...
  if (service.blkio_config !== undefined) {
    ...
    if (service.blkio_config.device_read_bps !== undefined) {
      let rate = service.blkio_config.device_read_bps[0].rate.toLowerCase();
      let device_read_bps = [{}];
      if (rate.includes("k")) {
        ...
        device_read_bps[0]["Path"] = service.blkio_config.device_read_bps[0].path;
        device_read_bps[0]["Rate"] = rate * 1024;
        output.BlkioDeviceReadBps = device_read_bps;
        ...
        device_read_bps[0]["Path"] = service.blkio_config.device_read_bps[0].path;
        device_read_bps[0]["Rate"] = rate * 1024 * 1024;
        output.BlkioDeviceReadBps = device_read_bps;
        ...
        device_read_bps[0]["Path"] = service.blkio_config.device_read_bps[0].path;
        device_read_bps[0]["Rate"] = rate * 1024 * 1024 * 1024;
        output.BlkioDeviceReadBps = device_read_bps;
      }
    }
    ...
    if (service.blkio_config.device_write_bps !== undefined) {
      let rate = service.blkio_config.device_write_bps[0].rate.toLowerCase();
      let device_write_bps = [{}];
      if (rate.includes("k")) {
        if (rate.indexOf("k") == rate.length - 1) {
          rate = rate.replace('k', '');
        } else if (rate.indexOf("k") == rate.length - 2) {
        ...
        device_write_bps[0]["Path"] = service.blkio_config.device_write_bps[0].path;
        device_write_bps[0]["Rate"] = rate * 1024;
        output.BlkioDeviceWriteBps  = device_write_bps;
      } else if (rate.includes("m")) {
        if (rate.indexOf("m") == rate.length - 1) {
          rate = rate.replace('m', '');
        } else if (rate.indexOf("m") == rate.length - 2) {
        ...
        device_write_bps[0]["Path"] = service.blkio_config.device_write_bps[0].path;
        device_write_bps[0]["Rate"] = rate * 1024 * 1024;
        output.BlkioDeviceWriteBps  = device_write_bps;
      } else if (rate.includes("g")) {
        if (rate.indexOf("g") == rate.length - 1) {
          rate = rate.replace('g', '');
        ...
        device_write_bps[0]["Path"] = service.blkio_config.device_write_bps[0].path;
        device_write_bps[0]["Rate"] = rate * 1024 * 1024 * 1024;
        output.BlkioDeviceWriteBps  = device_write_bps;
      }
    }
